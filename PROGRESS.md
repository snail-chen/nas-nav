...
- **界面升级 (UI Upgrade)**:
  - **赛博朋克风格登录页 (Cyberpunk Login)**: 
    - 彻底重构登录界面，采用 HUD (平视显示器) 风格。
    - 新增 **Matrix 数字雨** 动态背景 Canvas 特效。
    - 实现 **Glitch (故障)** 文字特效和扫描线动画。
    - 交互式输入框：焦点状态下显示动态光标和高亮边框。
    - 状态反馈：登录中、成功、失败均有专属动画效果。
  - **标题同步**:
    - 修复了浏览器标签页标题 (Document Title) 未跟随系统配置更新的问题。
    - 登录页标题现在会根据配置实时渲染，并带有故障艺术效果。
